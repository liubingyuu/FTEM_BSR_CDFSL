...
def entropy_loss(out_f):
    epsilon = 1e-16
    p = F.softmax(out_f, dim=1)
    ent = -torch.mean(torch.sum(p * torch.log(p + epsilon), 1))
    return ent
...
def finetune(novel_loader, n_query=15, freeze_backbone=False, n_way=5, n_support=5):
    ...
    for _, (x, y) in enumerate(novel_loader):
        ...
        rand_id_q = np.random.permutation(query_size)
        j_q = 0
        for epoch in range(total_epoch):
            ...
            for j in range(0, support_size, batch_size):
                ...
                #####################################
                bs_real = min(batch_size, support_size - j)
                selected_id = torch.from_numpy(rand_id[j: j + bs_real]).cuda()
                if j_q + bs_real > query_size:
                    rand_id_q = np.random.permutation(query_size)
                    j_q = 0
                selected_id_q = torch.from_numpy(rand_id_q[j_q: j_q + bs_real]).cuda()
                j_q += bs_real

                z_batch_s = x_a_i[selected_id]
                y_batch = y_a_i[selected_id]

                z_batch_q = x_b_i[selected_id_q]
                #####################################
                for i in range(params.M):
                    classifier_opt[i].zero_grad()
                    if freeze_backbone is False:
                        delta_opt[i].zero_grad()
                    output_s = pretrained_model[i](z_batch_s)
                    output_q = pretrained_model[i](z_batch_q)
                    output_s = classifier[i](output_s)
                    output_q = classifier[i](output_q)
                    loss = loss_fn(output_s, y_batch) + params.beta * entropy_loss(output_q)
                    #####################################
                    loss.backward()
        ...
        print('BSR+ENT (Ensemble): %f' % (correct_ori / count_this * 100))
        ...
        print('BSR+LP+ENT (Ensemble): %f' % (correct_lp / count_this * 100))
    ...
    print('BSR+ENT (Ensemble): %d Test Acc = %4.2f%% +- %4.2f%%' %
    ...
    print('BSR+LP+ENT (Ensemble): %d Test Acc = %4.2f%% +- %4.2f%%' %
